...
public class FactureServiceImpl implements IFactureService {
	...
	public Facture addDetailsFacture(Facture f, Set<DetailFacture> detailsFacture) {
		...
		for (DetailFacture detail : detailsFacture) {
			...
			Produit produit = produitRepository.findById(detail.getProduit().getIdProduit())
					.orElseThrow(()->new RuntimeException("Product with ID "+detail.getProduit().getIdProduit()+" was not found1" ));
	...
	public List<Facture> getFacturesByFournisseur(Long idFournisseur) {
		Fournisseur fournisseur = fournisseurRepository.findById(idFournisseur)
									.orElseThrow(()->new RuntimeException("Product with ID "+idFournisseur+" was not found2" ));
	...
	public void assignOperateurToFacture(Long idOperateur, Long idFacture) {
		Facture facture = factureRepository.findById(idFacture)
				.orElseThrow(()->new RuntimeException("Facture with ID "+idFacture+" was not found3" ));
		Operateur operateur = operateurRepository.findById(idOperateur)
				.orElseThrow(()->new RuntimeException("Operateur with ID "+idOperateur+" was not found4" ));
